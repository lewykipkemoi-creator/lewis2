...
const metrics = [
  ...
  { label: "AI conversations", value: "1,284", growth: "↑ 18.7%", tone: "text-white", growthTone: "text-emerald-400", icon: IconZap, bg: "bg-violet-500/10", iconTone: "text-violet-300" },
  { label: "Leads captured", value: "347", growth: "↑ 31.2%", tone: "text-white", growthTone: "text-emerald-400", icon: IconTarget, bg: "bg-fuchsia-500/10", iconTone: "text-fuchsia-300" },
];
...
export default function DashboardOverview() {
  ...
  return (
    <div className="max-w-6xl px-4 sm:px-6 lg:px-10 py-6 lg:py-8">
      <Reveal className="relative rounded-3xl border border-violet-500/20 overflow-hidden p-6 sm:p-8">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_85%_10%,rgba(139,92,246,0.22),transparent_45%)]" />
        <div className="absolute -right-24 -bottom-32 w-72 h-72 bg-violet-500 rounded-full blur-[100px] opacity-20" />
        <div className="relative flex flex-col sm:flex-row justify-between items-start gap-4">
          <div>
            <div className="text-violet-300 text-[11px] font-bold uppercase tracking-widest">AI Revenue Command Center</div>
            ...
            <div className="flex flex-wrap gap-3 mt-6">
              <button
                ...
                className="bg-gradient-to-r from-violet-500 to-violet-700 rounded-xl px-5 py-2.5 text-[13px] font-medium shadow-lg shadow-violet-600/25 hover:-translate-y-0.5 transition"
              >
                Run AI scan
              </button>
      ...
      <Reveal delay={100} className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-5">
        {metrics.map((m) => {
          const Icon = m.icon;
          return (
            <div key={m.label} className="rounded-2xl border border-white/8 bg-white/[0.02] p-4 hover:border-violet-500/25 hover:-translate-y-1 transition">
          ...
          <div className="h-40 sm:h-52 flex items-end gap-1.5 sm:gap-2.5">
            {[34, 45, 39, 61, 51, 73, 86, 94].map((h, i) => (
              <div key={i} className="flex-1 h-full flex items-end group relative">
                <div
                  style={{ height: `${h}%` }}
                  className="w-full rounded-t bg-gradient-to-t from-violet-700 to-violet-400 group-hover:brightness-125 transition"
                />
              </div>
            ))}
          </div>
                  ...
                  <tr key={lead.name} className="border-t border-white/6 text-[12px]">
                    <td className="py-2.5 px-1">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-600 to-fuchsia-500 flex items-center justify-center text-[10px] font-bold shrink-0">{lead.initials}</div>
          ...
          <div className="space-y-1">
            {activity.map((a, i) => (
              <div key={i} className="flex gap-2.5 rounded-lg p-2 hover:bg-white/[0.03] transition">
                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-[10px] font-bold shrink-0">L</div>
